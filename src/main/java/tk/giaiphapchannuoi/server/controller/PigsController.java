...
//        pigsDTO.set
    ...
    public ResponseEntity<Object> updatestatussold(@RequestBody List<Pigs> pigsList){
        ...
        for (Pigs temp :
                pigsList_temp) {
            response.add(new PigsDTO(temp.getId(),temp.getPigCode(),temp.getHouse().getId(),temp.getPigType(),temp.getBirthId(),temp.getBreed().getId(),temp.getGender(),temp.getBirthday(),temp.getBorn_weight(),temp.getBornStatus(),temp.getOriginId(),temp.getOriginFather(), temp.getOriginMother(),temp.getOriginWeight(),temp.getReceiveWeight(), temp.getHealthPoint(),temp.getFoot().getId(),temp.getFunctionUdder(),temp.getTotalUdder(),temp.getGentialType().getId(), temp.getDescription(),temp.getFcr(),temp.getAdg(),temp.getBf(),temp.getFilet(),temp.getLongBack(),temp.getLongBody(),temp.getIndex(),temp.getParities(),temp.getImages(),temp.getHealthStatus().getId(),temp.getBreedingType(),temp.getBreedStatus(),temp.getPregnancyStatus().getId(),temp.getPoint_review(),temp.getStatus().getId(),temp.getOverviewStatus(),temp.getDelFlag()));
        }
